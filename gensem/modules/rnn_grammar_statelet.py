...
class RnnGrammarStatelet(GrammarStatelet):
    ...
    def __init__(self,
        ...
        super().__init__(nonterminal_stack,
                         None,  # We do not need the valid_Actions field
                         is_nonterminal,
                         reverse_productions)
    ...
    def take_action(self,  # pylint: disable=arguments-differ
        ...
        ``rnn_state_stack``. The idea is that ``rnn_state`` is the output of an RNN cell after processing the
        production rule, whose right side contains the new nonterminals. When each of them is being expanded, the
        tree-encoder will want to use this RNN state as the input to the RNN cell. If the right side is a terminal,
        the ``rnn_state`` will not be stored.
        """
